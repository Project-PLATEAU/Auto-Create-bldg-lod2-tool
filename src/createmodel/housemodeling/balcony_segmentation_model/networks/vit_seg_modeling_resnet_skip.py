    ...
    def load_from(self, weights, n_block, n_unit):
        ...
        if hasattr(self, 'downsample'):
            ...
            proj_gn_weight = np2th(
                weights[pjoin(n_block, n_unit, "gn_proj/scale")])
            proj_gn_bias = np2th(
                weights[pjoin(n_block, n_unit, "gn_proj/bias")])
    ...
    def __init__(self, block_units, width_factor, in_channels=3):
        ...
        self.body = nn.Sequential(OrderedDict([
            ...
            ('block2', nn.Sequential(OrderedDict(
                ...
                [(f'unit{i:d}', PreActBottleneck(cin=width*8, cout=width *
                  8, cmid=width*2)) for i in range(2, block_units[1] + 1)],
            ))),
            ('block3', nn.Sequential(OrderedDict(
                [('unit1', PreActBottleneck(cin=width*8, cout=width*16, cmid=width*4, stride=2))] +
                [(f'unit{i:d}', PreActBottleneck(cin=width*16, cout=width *
                  16, cmid=width*4)) for i in range(2, block_units[2] + 1)],
            ))),
        ]))
